...
pub fn run_indexer(config: IndexerConfig) -> Result<()> {
    ...
    if config.generate_tsg {
        ...
        for language in &detected_languages {
            if language.get_stack_graphs_language().is_none() {
                if config.verbose {
                    info!(
                        "Generating TSG files for detected language: {}",
                        language.name()
                    );
                }
                ...
                match generator.generate(language) {
                    ...
                    Err(e) => {
                        warn!(
                            "Failed to generate TSG files for {}: {}",
                            language.name(),
                            e
                        );
                    }
    ...
    let mut symbols = indexer::SymbolIndex::new();

    // Index the path
    if config.path.is_file() {
        indexer::index_file(&mut stack_graph, &mut symbols, &config.path)
            .with_context(|| format!("Failed to index file: {}", config.path.display()))?;
    } else if config.path.is_dir() {
        indexer::index_directory(&mut stack_graph, &mut symbols, &config.path)
            .with_context(|| format!("Failed to index directory: {}", config.path.display()))?;
    }

    // Output the stack graph
    match config.format.as_str() {
        "json" => output::output_json(&stack_graph, &symbols, config.output.as_deref())?,
        "dot" => output::output_dot(&stack_graph, config.output.as_deref())?,
        _ => {
            warn!(
                "Unsupported output format: {}. Using JSON instead.",
                config.format
            );
            output::output_json(&stack_graph, &symbols, config.output.as_deref())?
        }
    }
